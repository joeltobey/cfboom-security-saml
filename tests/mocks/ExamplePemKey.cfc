 ...
 */
component
  displayname="Class ExamplePemKey"
  output="false"
{
  public void function enum() {
    this.RSA_TEST_KEY = new tests.mocks.ExamplePemKey(
    ...
    this.SP_RSA_KEY = new tests.mocks.ExamplePemKey(
      "-----BEGIN RSA PRIVATE KEY-----\n" &
    ...
    this.IDP_RSA_KEY = new tests.mocks.ExamplePemKey(
      "-----BEGIN RSA PRIVATE KEY-----\n" &
      "Proc-Type: 4,ENCRYPTED\n" &
  ...
  public tests.mocks.ExamplePemKey function init(string privatePem, string publicPem, string passphrase) {
    variables['_privatePem'] = arguments.privatePem;
    variables['_publicPem'] = arguments.publicPem;
    variables['_passphrase'] = arguments.passphrase;
    return this;
  }

  public cfboom.security.saml.key.SimpleKey function getSimpleKey(string name) {
    arguments['privateKey'] = getPrivate();
    arguments['certificate'] = getPublic();
    arguments['passphrase'] = getPassphrase();
    return new cfboom.security.saml.key.SimpleKey( argumentCollection = arguments );
  }

  public string function getPrivate() {
    return variables._privatePem;
  }

  public string function getPublic() {
    return variables._publicPem;
  }
  ...
  public cfboom.security.saml.key.SimpleKey function getPublicKey(string name) {
    arguments['certificate'] = getPublic();
    return new cfboom.security.saml.key.SimpleKey( argumentCollection = arguments );
  }
}
